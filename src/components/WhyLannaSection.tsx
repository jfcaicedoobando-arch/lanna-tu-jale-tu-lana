...
const FeatureCard = ({ icon: Icon, title, desc, index }: { icon: typeof MessageCircle; title: string; desc: string; index: number }) => {
  const { ref, isVisible } = useScrollAnimation(0.1);
  return (
    <div
      ref={ref}
      className={`scroll-fade-in ${isVisible ? "visible" : ""} p-6 sm:p-8 rounded-xl bg-card border border-border/50 hover:border-primary/30 transition-colors duration-300`}
      style={{ transitionDelay: `${index * 0.1}s` }}
    >
      <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center mb-4">
        <Icon className="w-6 h-6 text-primary" />
      </div>
      <h3 className="font-highway text-lg text-foreground mb-2">{title}</h3>
